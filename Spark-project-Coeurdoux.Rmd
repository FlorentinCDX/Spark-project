...
Here I download all the price files from 2013 to 2017, then I unzip them, assign a name to each of them and remove the zip file from the working directory.
```{r, eval = FALSE}
...
## Merge the distributed data frames together
...
Sadly the week function of the lubridate package is not working with spark because there is not equivqlent SQL queries. So to to avoid this problem I choose to build an aproximation of the week number by assuming that every month is made of 30 days. I also add the column day_year which count the number of the day in the year.
...
Here we observe that the E85 is way more expensive in the departement 48 (Lozere), and less in the deparement 50 (La Manche). 
```{r, warning=FALSE}
prixTot_ddf %>%
...
Before doeing the forecast I remove the potential NA of the data because sparklyr ml functions seems not to be working with NAs. I also split the model into a training and a test part. 
```{r}
...
Here is the random forest regression of price thancks to the day, the station id and the type of gas. We observe that Random forest model is as little less performent than the linear regression model.
```{r}
